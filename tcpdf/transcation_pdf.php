...
$view_data = $db->query("SELECT * FROM `account_transaction` WHERE `account_id` = '$account_id' AND deleted = '0'ORDER BY id DESC  limit 43 ");

$add_br  = '';
$number_loop_count = $view_data->rowCount() % 20;

if($number_loop_count > 11 ){
    $add_br = '<br><br><br><br><br><br><br>';
}
...
if( $view_data->rowCount() > 0  ) {
 ...
 foreach ($MIAN_CURRENCIES as $key => $value) {
    ...
    AND `type` = 'credit' AND `account_id` = '$account_id' AND `currency_id` = $key  ")->fetch()['amount'] + 0 ;

    $debt = $db->query("SELECT SUM(amount) as amount FROM `account_transaction` WHERE deleted = 0 AND `type` = 'debt' AND `account_id` = '$account_id' AND `currency_id` = $key")->fetch()['amount'] + 0 ;
