...
$view_data = $db->query("SELECT * FROM `accounts` WHERE `deleted` = '0' and `type` = 'public' $condition ORDER BY id DESC LIMIT $to OFFSET $from ");

    $list_data  = $db->query("SELECT count(id) as record FROM `accounts` WHERE deleted = 0 and `type` = 'public' $condition ")->fetch();
        ...
        <?php 
              ...
              <div class="card">
                <div class="card-body">
                  <div class="align-items-center row">
                    <div class="col">
                      <h5 class="text-start lalezar text-muted mb-2">ثبت   حساب</h5>
                      <div class="row mb-4">
                        <!-- buy form start -->
                        <div class="col-2"></div>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-12">
                        <form method="post" action="" role="form" class="row">
                            ...
                            <div class="col-xs-12 col-md-6 col-lg-2 mb-3">
                                <label for="exampleInputPassword1" class="form-label text-muted"> شماره تماس</label>
                                <input type="text" autocomplete="off" id="s_phone" name="s_phone" value="" onload ="" class="form-control " >
                            </div>
                            <div class="col-xs-12 col-md-6 col-lg-2 mb-3">
          ...
          <div class="col-xl-12 mt-4">
            <div class="card">
              <div class="card-body">
                <div class="row mt-2">
                  <h5 class="text-start lalezar text-muted mb-3 ms-3">حسابات</h5>
                            ...
                            foreach ($view_data as $rows) {
                                ...
                                echo '
                                    <tr class="active">
                                        ...
                                        <td class = ""> ' . $rows['last_name'] . ' </td>
                                        <td class = ""> ' . $rows['fname'] . ' </td>
                                        <td class = ""> ' . $rows['phone'] . ' </td>
                                        <td class = ""> ' . $rows['address'] . ' </td>
                                        <td class = ""> '.$rows['date']. ' </td>
                                        <td class = ""> ' . $rows['note'] . ' </td>
                                        <td class = ""> ' . get_user_name($rows['user_id']) . ' </td>
                                        <td class="tab-cen center">
                                            <div class="visible-md visible-lg hidden-sm hidden-xs">
                                                <a href="edit_account.php?id='.base64_encode($rows['id']).'" class="btn btn-xs btn-teal tooltips" data-placement="top" data-original-title="Edit">
                                                    <i class="fa fa-edit"></i>
                                                </a>
                                                <a href="action_account.php?delete&id='.base64_encode($rows['id']).'&pagename=add_account" class="btn btn-xs btn-bricky tooltips deleted" data-placement="top" data-original-title="Delete">
